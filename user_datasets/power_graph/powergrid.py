# This is a copy from https://github.com/PowerGraph-Datasets/PowerGraph-Graph/blob/main/code/dataset/powergrid.py
#  with slight changes for automatic downloading
...
class PowerGrid(InMemoryDataset):
    ...
    def __init__(self, root, name, datatype='Binary', transform=None, pre_transform=None,
                 pre_filter=None):
    ...
    def process(self):
        ...
        for i in range(len(node_f)):
            ...
            if self.datatype.lower() == 'multiclass':
                # do argmax
                ydata = torch.tensor(np.argmax(of_mc[i][0]), dtype=torch.float, device=device).view(
                    1, -1)
