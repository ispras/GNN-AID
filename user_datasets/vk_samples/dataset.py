...
class AttrInfo:
    ...
    def attribute_vals(
        ...
        # Check if cached
        if (graph_name, attribute) in AttrInfo._attribute_vals_cache:
            return AttrInfo._attribute_vals_cache[(graph_name, attribute)]
        ...
        res = None
        if graph_name == 'vk_samples':
            res = vk_dict[attribute]

        elif graph_name == ('attributed', 'vk_10_classes'):
            ...
            raise NotImplementedError(f"Attribute values for graph {graph_name}")

        AttrInfo._attribute_vals_cache[(graph_name, attribute)] = res
        return res

    @staticmethod
    def one_hot(
            graph_name: Union[str, tuple],
            attribute: Union[str, tuple, list],
            value: int,
            add_none: bool = False
        ...
        allowed_vals = AttrInfo.attribute_vals(graph_name, attribute)
...
class VKSampleDataset(
    ...
    def __init__(
        ...
        """
        :param add_none: flag indicating whether to add a value under the none class in the feature
         vector.
        """
        self.add_none = add_none

        super(VKSampleDataset, self).__init__(dataset_config)
    ...
    def node_attributes(
        ...
        for a in attrs:
            if a not in self._node_attributes:
                # If we want age, convert it to number first
                if a == 'age':
                    self.bdate_to_age(attr_dir_path=self.node_attributes_dir,
                                      node_map=self.node_map)
                # Read from file and extract proper values
                parts = a.split(',')
                with open(self.node_attributes_dir / Path(parts[0]), 'r') as f:
                    ...
                    if len(parts) > 1:
                        if parts[0] == 'schools':
                            ...
                            attr_dict = {
                                key: value.get(parts[1], None) for key, value in attr_dict.items()
                            }
                self._node_attributes[a] = []
                node_attributes = {}
                for ix, orig in self._iter_nodes():
                    node_attributes[ix] = attr_dict[orig]
...
if __name__ == '__main__':
    ...
    dc = DatasetConfig(('example', 'custom', 'vk_samples', graph_name))
    dataset = VKSampleDataset(dc)

    dataset.set_visible_part({'center': 1, 'depth': 0})
