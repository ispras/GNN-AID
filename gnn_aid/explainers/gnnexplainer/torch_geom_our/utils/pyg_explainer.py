...
from experiments.various_tasks import link_prediction
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.explanation_type == ExplanationType.phenomenon:
            ...
            link_prediction_task = kwargs.pop('link_prediction')
            if link_prediction_task is True:
                node_embeddings = self.model(x, edge_index)
                src = node_embeddings[index[0]]
                dst = node_embeddings[index[1]]
                # TODO use model.get_answer() function when Misha D. done
                prediction = self.model.decode(src, dst).sigmoid()
                target = (prediction.squeeze(-1) >= 0.5).to(torch.int64)
            else:
