...
class ExplainerMetric:
    def __init__(self, explainers_manager: Type, explaining_metrics_params=None):
        self.obj_id_to_explainer_run_config = None
    ...
    def evaluate(self, obj_id_to_explainer_run_config: dict) -> dict:
        self.obj_id_to_explainer_run_config = obj_id_to_explainer_run_config
        target_objs_indices = sorted(obj_id_to_explainer_run_config.keys())

        self.get_explanations(target_objs_indices[0])
        ...
        stability = []
        consistency = []
        for obj_ind in target_objs_indices:
            print(f"Processing explanation metrics calculation for obj id {obj_ind}.")
            self.get_explanations(obj_ind)
            sparsity += [self.calculate_sparsity(obj_ind)]
        ...
        fidelity = self.calculate_fidelity(target_objs_indices)
    ...
    def get_explanations(self, obj_ind, num_explanations=1) -> list[dict]:
        explanations = []
        for explanation_index in range(num_explanations):
            run_config = self.obj_id_to_explainer_run_config[obj_ind]
            ...
            if os.path.exists(explainer_result_file_path):
                ...
                explanation = self.calculate_explanation(run_config, self.gen_dataset, save_explanation_flag=True)
    ...
    def filter_graph_by_explanation(
        ...
        important_nodes, important_edges = ExplainerMetric.parse_explanation(explanation)
    ...
    def calculate_explanation_vectors(
        ...
        base_important_nodes, base_important_edges = ExplainerMetric.parse_explanation(
            base_explanation
        )
        perturbed_important_nodes, perturbed_important_edges = ExplainerMetric.parse_explanation(
...
class NodesExplainerMetric(ExplainerMetric):
    def __init__(self, explainers_manager: Type, explaining_metrics_params=None):
        super().__init__(explainers_manager=explainers_manager, explaining_metrics_params=explaining_metrics_params)

        self.graph = explainers_manager.gen_dataset.dataset.data
        self.x = self.graph.x
...
class GraphExplainerMetric(ExplainerMetric):
    def __init__(self, explainers_manager: Type, explaining_metrics_params=None):
        super().__init__(explainers_manager=explainers_manager, explaining_metrics_params=explaining_metrics_params)
    ...
    def calculate_fidelity(
            self,
            target_graph_indices: list
    ) -> list[int]:
        same_answers_count = []
        for graph_ind in target_graph_indices:
            self.graph = self.gen_dataset.dataset[graph_ind]
            self.x = self.graph.x
            self.edge_index = self.graph.edge_index
            original_answer = self.model.get_answer(self.x, self.edge_index)
            ...
            filtered_answer = self.model.get_answer(new_x, new_edge_index)
            matched = filtered_answer == original_answer
            print(f"Processed fidelity calculation for graph id {graph_ind}. Matched: {matched}")
            same_answers_count.append(int(matched))
    ...
    def calculate_sparsity(
        ...
        explanation = self.get_explanations(graph_ind)[0]
        if "data" not in explanation:
            raise Exception(f"Invalid explanation. No data. Explanation: {explanation}")
        explanation_data = explanation["data"]
        node_cnt = self.x.shape[0]
        edge_cnt = self.edge_index.shape[1]
        num = 0
        den = 0
        if "nodes" in explanation_data and explanation_data["nodes"] and len(explanation_data["nodes"]) != 0:
            num += len(explanation["data"]["nodes"])
        ...
        if den == 0:
            ...
            # raise Exception(f"Invalid explanation. No data. Explanation: {explanation}")
        sparsity = 1 - num / den
        print(f"Sparsity calculation for graph id {graph_ind} completed.")
        return sparsity
    ...
    def calculate_stability(
        ...
        print(f"Stability calculation for graph id {graph_ind} started.")
        self.graph = self.gen_dataset.dataset[graph_ind]
        self.x = self.graph.x
        self.edge_index = self.graph.edge_index

        base_explanation = self.get_explanations(graph_ind)[0]
        run_config = self.obj_id_to_explainer_run_config[graph_ind]
        stability = []
        if graph_ind not in self.dictionary["perturbed_explanations"]:
            self.dictionary["perturbed_explanations"][graph_ind] = []

        for i in range(graph_perturbations_nums):
            if i < len(self.dictionary["perturbed_explanations"][graph_ind]):
                perturbed_explanation = self.dictionary["perturbed_explanations"][graph_ind][i]
            else:
                new_dataset = self.perturb_graph(
                    self.gen_dataset, graph_ind, feature_change_percent, node_removal_percent
                )
                perturbed_explanation = self.calculate_explanation(run_config, new_dataset)
                self.dictionary["perturbed_explanations"][graph_ind] += [perturbed_explanation]
                self.save_dictionary()
        ...
        # stability = stability / graph_perturbations_nums
        print(f"Stability calculation for node id {graph_ind} completed.")
        return stability
    ...
    def calculate_consistency(
        ...
        print(f"Consistency calculation for graph id {graph_ind} started.")

        self.graph = self.gen_dataset.dataset[graph_ind]
        self.x = self.graph.x
        self.edge_index = self.graph.edge_index

        explanations = self.get_explanations(graph_ind, num_explanations=num_explanation_runs + 1)
        explanation = explanations[0]
        consistency = []
        for ind in range(num_explanation_runs):
            perturbed_explanation = explanations[ind + 1]
        ...
        # consistency = consistency / num_explanation_runs
        print(f"Consistency calculation for grsph id {graph_ind} completed.")
        return consistency
    ...
    def perturb_graph(
        ...
        graph = new_dataset.dataset[graph_ind]
        x = graph.x.clone()
        ...
        num_features_to_change = int(feature_change_percent * num_nodes * num_features)
        indices = torch.randint(0, num_nodes * num_features, (num_features_to_change,), device=x.device)
        x.view(-1)[indices] = 1.0 - x.view(-1)[indices]

        num_nodes_to_remove = int(node_removal_percent * num_nodes)

        if num_nodes_to_remove > 0:
            nodes_to_remove = torch.randperm(num_nodes, device=edge_index.device)[:num_nodes_to_remove]
        ...
        new_dataset.dataset[graph_ind].x = graph.x
        new_dataset.dataset[graph_ind].edge_index = graph.edge_index
